...
- [SAM — Gerador de Dados Sintéticos](#sam--gerador-de-dados-sintéticos)
  - [Índice](#índice)
  ...
  - [Importação para a Base de Dados](#importação-para-a-base-de-dados)
    ...
    - [MongoDB — Mongoose](#mongodb--mongoose)
      ...
      - [2. Abrir o túnel SSH](#2-abrir-o-túnel-ssh)
      - [3. Importar os dados](#3-importar-os-dados)
...
O MongoDB não está exposto diretamente ao exterior — o acesso remoto é feito via **túnel SSH**. É necessário ter acesso SSH ao servidor onde o MongoDB está instalado.

#### 1. Configurar o `.env`

As credenciais MongoDB devem ser definidas como variáveis separadas (a password pode conter caracteres especiais que não são compatíveis com URI):

```env
MONGODB_URI=mongodb://127.0.0.1:27017/Grupo02_Mongo
MONGODB_DB_NAME=Grupo02_Mongo
MONGODB_USER=utilizador
MONGODB_PASSWORD=password
MONGODB_AUTH_SOURCE=Grupo02_Mongo
```

#### 2. Abrir o túnel SSH

Num terminal separado, redirecionar a porta do MongoDB para localhost:

```bash
ssh -L 27017:localhost:27017 utilizador@servidor -N
```

O `-N` não executa nenhum comando — o terminal fica em silêncio enquanto o túnel está ativo. Não fechar este terminal durante a importação.

> **Conflito de porta:** se a porta 27017 já estiver em uso localmente (ex: MongoDB local instalado), usar uma porta alternativa:
>
> ```bash
> ssh -L 27018:localhost:27017 utilizador@servidor -N
> ```
>
> E atualizar o `MONGODB_URI` no `.env` para usar `:27018`.
